...
<head>
    ...
    <title>Modifier la réservation - {{ $house->description }}</title>
...
<body>
    <div class="container">
        ...
        <div class="breadcrumb">
            <a href="/">Accueil</a> &gt; <a href="{{ route('reservations.index') }}">Mes réservations</a> &gt; Modifier
        </div>
        ...
        <div class="main-content">
            <div class="property-section">
                ...
                <div id="success-message" class="success-message hidden">
                    ...
                    Votre réservation a été modifiée avec succès !<br>
                ...
                <form id="reservationForm">
                        ...
                        <div class="date-row">
                            <div class="date-input-group">
                                ...
                                <input type="date" class="date-input" id="start_date" name="start_date" 
                                       value="{{ $reservation->start_date->format('Y-m-d') }}" required>
                            </div>
                            <div class="date-input-group">
                                <label for="end_date">DÉPART</label>
                                <input type="date" class="date-input" id="end_date" name="end_date" 
                                       value="{{ $reservation->end_date->format('Y-m-d') }}" required>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="options-container">
                            ...
                            <label class="option-checkbox">
                                <input type="checkbox" id="has_breakfast" name="has_breakfast" value="1" 
                                       {{ $reservation->has_breakfast ? 'checked' : '' }}>
                            ...
                            <label class="option-checkbox">
                                <input type="checkbox" id="has_love_room" name="has_love_room" value="1" 
                                       {{ $reservation->has_love_room ? 'checked' : '' }}>
                                <span class="checkbox-custom"></span>
                                <span class="option-label-text">
                                    Chambre romantique
                            ...
                            @if($house->additional_options && count($house->additional_options) > 0)
                                @php
                                    $reservationOptions = $reservation->additional_options ?? [];
                                    $reservationOptionNames = collect($reservationOptions)->pluck('name')->toArray();
                                @endphp
                                ...
                                <label class="option-checkbox" style="border-top: 1px solid rgba(221, 221, 221, 0.5); padding-top: 12px; margin-top: 4px;">
                                    <input type="checkbox" 
                                           ...
                                           {{ in_array($option['name'], $reservationOptionNames) ? 'checked' : '' }}>
                                    <span class="checkbox-custom"></span>
                                    <span class="option-label-text">
                    ...
                    <div class="guests-section">
                        ...
                        <textarea class="guests-input" id="notes" name="notes" 
                                placeholder="Ajoutez vos demandes spéciales ou commentaires...">{{ $reservation->notes ?? '' }}</textarea>
                    </div>

                    <div id="availability-status"></div>

                    <button type="submit" class="reserve-button" id="submitBtn">
                        <span id="submitText">Enregistrer les modifications</span>
                        <span id="spinner" class="spinner hidden"></span>
                    </button>

                    <div class="charge-notice">
                        Vous pourrez finaliser le paiement après modification
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialiser les valeurs min/max pour endDate
            ...
            async function checkAvailability() {
                ...
                try {
                    ...
                    // Note: La vérification côté serveur devrait exclure cette réservation, mais on vérifie quand même
                    if (data.available) {
                        availabilityStatus.innerHTML = `
                    ...
                    `;
                    submitBtn.disabled = false; // Permettre quand même la soumission en cas d'erreur
                }
            }
